...
const SearchClient = ({ prompt }) => {
    ...
    const fetchResults = async () => {
      ...
      try {
        ...
        normalized = normalized.map((candidate, index) => ({
          ...
          skills: candidate.skills || candidate.technologies || candidate.techStack || '',
          experience: candidate.experience || candidate.yearsOfExperience || candidate.experienceYears || '',
          ...candidate
        }));
  ...
  const handleNext = () => {
    if (selectedCandidates.length === 0) {
      alert('Please select at least one candidate to find similar professionals.');
      return;
    }
    
    // Navigate to similar candidates page with selected candidates
    const candidatesParam = encodeURIComponent(JSON.stringify(selectedCandidates));
    ...
    <div className="flex-1 p-6">
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-2">
          <h1 className="text-2xl font-bold text-gray-900">Select a Reference Candidate</h1>
        ...
        <p className="text-gray-600">Choose one or more candidates to find similar professionals.</p>
      </div>
      ...
      <div className="space-y-4">
        ...
        {!loading && results.length === 0 && (
          <div className="text-center py-8">
            <div className="text-gray-600">No candidates found for your search. Please try a different search term.</div>
          </div>
        )}

        {!loading && results.length > 0 && results.map((candidate) => {

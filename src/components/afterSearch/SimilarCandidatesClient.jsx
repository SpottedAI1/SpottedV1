  ...
  useEffect(() => {
    ...
    const fetchSimilarCandidates = async () => {
      ...
      try {
        ...
        const candidateSkills = selectedCandidates.map(c => c.skills).join(' | ');
        ...
        if (Array.isArray(data)) {
          ...
          else if (data.candidates && Array.isArray(data.candidates)) normalized = data.candidates;
    ...
    <div className="flex-1 p-6">
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-2">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Similar Candidates</h1>
            <p className="text-gray-600 mt-1">Based on: {referenceNames}</p>
          </div>
          <div className="flex items-center gap-4">
            <span className="text-sm text-gray-500">
              {loading ? 'Loading...' : `Showing 1-${results.length} of ${results.length} candidates`}
            </span>
            <button 
      ...
      <div className="max-h-[calc(100vh-200px)] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100">
        <div className="space-y-4 pr-2">
          ...
          {!loading && results.length === 0 && (
            <div className="text-center py-8">
              <div className="text-gray-600">No similar candidates found. Please try selecting different reference candidates.</div>
            </div>
          )}
